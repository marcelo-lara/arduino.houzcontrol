...
void handleIrCode(unsigned long irCode)
{
	...
	//turn light on
	case sonyIrDvrCenter:
		houz.pushData(CMD_SET, suite_light, 2);
		break;

	//fan control
	case sonyIrDvr1:
		houz.pushData(CMD_SET, suite_fan, 1);
		break;
	case sonyIrDvr2:
		houz.pushData(CMD_SET, suite_fan, 2);
		break;
	case sonyIrDvr3:
		houz.pushData(CMD_SET, suite_fan, 3);
		break;
	case sonyIrDvr4:
		houz.pushData(CMD_SET, suite_fan, 4);
		break;
	case sonyIrDvr0:
		houz.pushData(CMD_SET, suite_fan, 0);
		break;
	...
	//push enviroment
	case sonyIrDvrUp:
		houz.pushData(CMD_QUERY, suite_enviroment, 0);
		break;

	//AC
	case sonyIrDvrA:
		houz.pushData(CMD_SET, suite_AC, 24);
		break;
	case sonyIrDvrB:
		houz.pushData(CMD_SET, suite_AC, 0);
		break;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Lighting control
void setMainLight(int state)
{ //todo: check this..
	if (state > 1)
		state = !digitalRead(mainLight) == 0 ? 1 : 0;
